(function($) {
    ...
    var APP = {
        ...
        isMobile: function() { return window.matchMedia('(max-width: 1199px)').matches; },
        ...
        processTestimonials: function() {
            ...
            if ($slick_slider.length < 1) { return; }
            $slick_slider.on('init', function(slick) { $(this).find('.slick-current').prev().addClass('prev'); });
        ...
        parallaxImag: function() {
            ...
            image_wrapper.on('mouseleave', (function(e) {
                e.preventDefault();
                $.each(image_wrapper.find('.layer'), function() { TweenMax.to($(this), 1, { x: 0, y: 0 }); });
            }));
        },
    };
    APP.slickSlider = {
        init: function($wrap) { this.slickSetup($wrap); },
        slickSetup: function($wrap) {
            var $slicks;
            if ($wrap !== undefined) { $slicks = $wrap } else { $slicks = $('.slick-slider'); }
            var options_default = { slidesToScroll: 1, slidesToShow: 1, adaptiveHeight: true, arrows: true, dots: true, autoplay: false, autoplaySpeed: 3000, centerMode: false, centerPadding: "50px", draggable: true, fade: false, focusOnSelect: false, infinite: false, pauseOnHover: false, responsive: [], rtl: false, speed: 300, vertical: false, prevArrow: '<div class="slick-prev" aria-label="Previous"><i class="far fa-angle-left"></i></div>', nextArrow: '<div class="slick-next" aria-label="Next"><i class="far fa-angle-right"></i></div>', customPaging: function(slider, i) { return $('<span></span>'); } };
            $slicks.each(function() {
                var $this = $(this);
                if (!$this.hasClass('slick-initialized')) {
                    var options = $this.data('slick-options');
                    if ($this.hasClass('custom-slider-1')) { options.customPaging = function(slider, i) { return '<span class="dot">' + (i + 1) + '</span>' + '<span class="dot-divider"></span><span class="dot">' + slider.slideCount + '</span>'; } }
                    ...
                    $this.on('setPosition', function(event, slick) {
                        var max_height = 0;
                        slick.$slides.each(function() { var $slide = $(this); if ($slide.hasClass('slick-active')) { if (slick.options.adaptiveHeight && (slick.options.slidesToShow > 1) && (slick.options.vertical === false)) { if (max_height < $slide.outerHeight()) { max_height = $slide.outerHeight(); } } } });
                        if (max_height !== 0) { $this.find('> .slick-list').animate({ height: max_height }, 500); }
                    });
                }
            });
        }
    ...
    APP.util = {
                ...
                var $this = $(this),
                    defaults = {
                        ...
                        callbacks: {
                            ...
                            beforeClose: function() { this.container.trigger('gtf_mfp_beforeClose'); },
                            close: function() { this.container.trigger('gtf_mfp_close'); },
                            change: function() {
                                var _this = this;
                                if (this.isOpen) {
                                    this.wrap.removeClass('mfp-ready');
                                    setTimeout(function() { _this.wrap.addClass('mfp-ready'); }, 10);
                                }
                            }
                        }
                    },
        ...
        tooltip: function($elWrap) {
            ...
            $elWrap.find('[data-toggle="tooltip"]').each(function() {
                ...
                if ($(this).closest('.gtf__tooltip-wrap').length) { configs = $.extend({}, configs, $(this).closest('.gtf__tooltip-wrap').data('tooltip-options')); }
            ...
            $chartEls.each(function() {
                ...
                if (typeof Waypoint !== 'undefined') {
                    $('.chart').waypoint(function() {
                        var myChart = new Chart($this, { type: chatType, data: { labels: labels, datasets: datasets }, options: options });
                        $(window).resize(function() { myChart.update(); });
                        $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) { myChart.update(); });
                        $('.collapsible').on('show.bs.collapse', function() { myChart.update(); });
                    }, { triggerOnce: true, offset: 'bottom-in-view' });
                }
            });
        }
    ...
    APP.CollapseTabsAccordion = {
        init: function() { this.CollapseSetUp(); },
        CollapseSetUp: function() {
            ...
            $tabs.find('.collapsible').on('show.bs.collapse', function() {
                ...
                if (!$tab_pane.hasClass('nested-tab-pane')) { $this_tab.find('.tab-pane').removeClass('show active'); }
                $parent.addClass('show active');
                var $nav_link = $parent.parents('.collapse-tabs').find('.nav-link');
                if (!$nav_link.hasClass('nested-nav-link')) { $nav_link.removeClass('active'); }
    ...
    APP.animation = {
        ...
        init: function() {
            ...
            if (typeof delay !== 'undefined') { _self.delay = delay; }
            _self.itemQueue["animated_0"] = [];
            $body.find('#content').find('>div,>section').each(function(index) {
                $(this).attr('data-animated-id', (index + 1));
                _self.itemQueue["animated_" + (index + 1)] = [];
            });
            setTimeout(function() { _self.registerAnimation(); }, 200);
        },
            ...
            $('[data-animate]:not(.animated)', _self.$wrapper).waypoint(function() {
                ...
                if ($this.is(":visible")) {
                    var $animated_wrap = $this.closest("[data-animated-id]"),
                        animated_id = '0';
                    if ($animated_wrap.length) { animated_id = $animated_wrap.data('animated-id'); }
        ...
        processItemQueue: function() {
            ...
            _self.queueTimer = window.setInterval(function() {
                var has_queue = false;
                for (var animated_id in _self.itemQueue) { if (_self.itemQueue[animated_id].length) { has_queue = true; break; } }
                if (has_queue) {
    ...
    APP.ShowCompare = {
        init: function() {
            ...
            btn_show.on("click", function(event) { event.preventDefault(); if ($(this).parent('#compare').hasClass('show')) { $(this).parent('#compare').removeClass('show'); } else { $(this).parent('#compare').addClass('show'); } });
        },
    };
    APP.headerSticky = {
        ...
        sticky: function() {
            $('.header-sticky .sticky-area').each(function() {
                ...
                if (window.matchMedia('(max-width: 1199px)').matches) { $nav_dashbard.addClass('header-sticky-smart'); } else { $nav_dashbard.removeClass('header-sticky-smart'); }
            });
        },
        resize: function() {
        ...
        scroll: function() { $window.on('scroll', function() { APP.headerSticky.processSticky(); }); },
        processSticky: function() {
            var current_scroll_top = $window.scrollTop();
            var $parent = $('.main-header');
            var is_dark = false;
            if ($parent.hasClass('navbar-dark') && !$parent.hasClass('bg-secondary')) { is_dark = true; }
            $('.header-sticky .sticky-area').each(function() {
                var $this = $(this);
                if (!$this.is(':visible')) { return; }
                var $wrap = $this.parent(),
                ...
                if (borderWidth !== '') { sticky_top += parseInt(borderWidth); }
    ...
    APP.invoice = {
        ...
        checkAllListing: function() {
            ...
            if ($parent_check.length < 1) { return; }
            var $child_checks = $('.child-chk');
            $parent_check.on('click', function() { $child_checks.prop('checked', $(this).prop('checked')); });
        },
        addItem: function() {
            var $btn_add = $('.btn-invoice-add-item');
            if ($btn_add.length < 1) { return; }
        ...
        currencySelect: function() {
            ...
            if ($currency_select.length < 1) { return; }
        ...
        taxSelect: function() {
            ...
            $items.each(function() {
                $(this).on('click', function(e) {
                    ...
                    if ($self.data('value') !== 0) { $('.tax-rate').css('display', 'block'); } else { $('.tax-rate').css('display', 'none'); }
                })
            });
        },
        discountSelect: function() {
            var $discount_select = $('.invoice-discount-select');
            if ($discount_select.length < 1) { return; }
            ...
            $items.each(function() {
                $(this).on('click', function(e) {
                    ...
                    if ($self.data('value') !== 0) { $('.discount-amount').css('display', 'block'); } else { $('.discount-amount').css('display', 'none'); }
                })
            });
        },
        printInvoice: function() { $('.invoice-action-print').on('click', function() { window.print(); }); },
        sortingInvoice: function() {
            var $table = $('#invoice-list');
            $table.DataTable({ "order": [], "paging": false, "searching": false, "info": false, "columnDefs": [{ "orderable": false, "targets": 0 }, { "targets": 'no-sort', "orderable": false }] });
        }
    };
